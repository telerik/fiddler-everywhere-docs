...
Any user can download Fiddler Reporter and start it immediatelly without the need for an explicit installation. After the tool loads, you can choose and start the prefered [capturing mode]({%slug reporter-features%}) to log the targeted HTTPS traffic. Once the capturing completes, the Fiddler Reporter tool presents the option to save the captured traffis as a SAZ file (also known as an Fiddler archive). The SAZ archive will contain a snapshot of all captured HTTPS sessions, which, at any point, can be loaded in Fiddler Everywhere.
 ...
 - Start the application by clicking on the downloaded artefact.
...
Everywhere Reporter starts an independent browser instance (Google Chrome or MS Edge) preconfigured to respect the Fiddler proxy and to trust its certificate authority (CA). If you aim to capture traffic from other client you can use other [capturing modes]({%slug reporter-features%}).
...
- Click on the **Save Capture** option and choose location to store your SAZ file.

That's it! You have now captured, saved and exported the web traffic as Fiddler archive (SAZ). As a next step a licensed Fiddler user can load the SAZ archive in the Fiddler Everywhere desktop application where the traffic can be inspected, analyzed, replayed, and modified as per your needs.
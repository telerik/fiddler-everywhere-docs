...
Follow these steps to configure Fiddler Everywhere and your Android device for remote traffic capture.
...
### Configuring Fiddler Proxy on Android Device
...
2. **Configure the Fiddler Everywhere proxy on the Android device:**
    ...
    1. Choose **Manual proxy**.
        - Enter the IP address of the Fiddler Everywhere host (for example, `192.168.100.50`).
...
## Configuring Fiddler Proxy on Android Emulator

Android Virtual Devices (AVDs), also known as Android emulators, can be configured to use Fiddler Everywhere as a proxy by adjusting the network settings within the emulator, similar to a physical device. The key difference is that the proxy address for the emulator must be set to its loopback address. For most standard Android emulators, this address is `10.0.2.2`, but some third-party emulators may use a different loopback alias. Refer to your emulator's documentation if you are unsure.

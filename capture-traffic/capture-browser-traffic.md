...
meta_title: Capturing from Independent Browser Instance | Fiddler Everywhere
description: "Capture browser HTTPS traffic with Fiddler Everywhere and the independent browser capturing mode. Capture, inspect, and analyze HTTPS sessions in real time."
...
As a result, Fiddler Everywhere starts an independent browser instance preconfigured to respect the Fiddler proxy and to trust its root CA. You can type an arbitrary URL, and the HTTPS traffic generated from its requests and response will immediately appear in the Fiddler Live Traffic grid.
